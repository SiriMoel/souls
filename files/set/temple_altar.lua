...
function spawn_all_shopitems( x, y )
	...
	SetRandomSeed( x, y )
	local count = tonumber( GlobalsGetValue( "TEMPLE_SHOP_ITEM_COUNT", "6" ) )
	local width = 132
    ...
    for i=1,count do
        if( i == sale_item_i ) then
            generate_shop_item( x + (i-1)*item_width, y, true, nil, true, false )
			generate_shop_wand( x + (i-1)*item_width, y - 60, true )
        else
            generate_shop_item( x + (i-1)*item_width, y, false, nil, true, false )
			generate_shop_wand( x + (i-1)*item_width, y - 60, false )
        end
        
        generate_shop_item( x + (i-1)*item_width, y - 25, false, nil, true, true )
